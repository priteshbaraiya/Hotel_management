    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div class="max-w-md w-full space-y-8 bg-white/10 backdrop-blur-lg p-10 rounded-2xl shadow-2xl border border-white/20">
        <div class="text-center">
          <h2 class="text-3xl font-serif font-bold text-white">Forgot Password</h2>
        ...
        @if (successMessage()) {
          <div class="bg-green-500/20 border border-green-500/50 text-green-200 px-4 py-3 rounded-lg text-sm">
            {{ successMessage() }}
            <p class="mt-2 text-gray-300">Check the backend console for the reset link.</p>
          </div>
          <a routerLink="/login" class="block text-center font-medium text-gold-400 hover:text-gold-300 transition">
            Back to Login
          </a>
        } @else {
            ...
            this.authService.forgotPassword(this.forgotForm.value.email).subscribe({
                ...
                error: (err) => {
                    ...
                    if (err.status === 0) {
                        ...
                        this.errorMessage.set(err.error?.msg || 'Failed to send reset email');
                    }
                }
            });
        }
